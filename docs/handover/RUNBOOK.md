# دليل التشغيل — ALTA HOST (§14-5)

كل العمليات أدناه تتم عبر الواجهة أو الـ API الموثّق في `/api/docs`.

## إنشاء فندق جديد (§13)
من شاشة «إدارة المنصة» بحساب `alta_admin` (المستخدم التجريبي: alta):
نموذج واحد ينشئ الفندق + المستأجر (تلقائيًا عبر مشغّل قاعدة البيانات) + الباقة
وحصة التخزين + حساب مدير الفندق. سلّم بيانات دخول المدير بقناة آمنة —
كلمة المرور لا تُخزَّن إلا مشفّرة (bcrypt) ولا تُعرض مرة أخرى.
من الشاشة نفسها: تغيير الباقة والحصة، والإيقاف/إعادة التفعيل — الإيقاف
يقطع وصول موظفي الفندق فورًا (بياناته تبقى سليمة).

## ربط واتساب
- **وضع التجربة:** بدون أي إعداد — المحاكي يمر بكامل الخط.
- **إنتاج (WhatsApp Cloud API):** من شاشة بيانات الاعتماد خزّن
  `whatsapp.cloudApiToken` و `whatsapp.phoneNumberId` (تُخزَّن مشفّرة AES-256-GCM، لا تُعرض أبدًا).
  ثم وجّه Webhook ميتا إلى `POST /webhook/whatsapp`.

### تجربة واتساب محليًا عبر WAHA
1. في `.env`: `WHATSAPP_PROVIDER=waha` و `WAHA_API_KEY=<مفتاح ثابت من اختيارك>`.
   **مهم:** WAHA يولّد مفتاحًا عشوائيًا عند كل إقلاع ويرفض كل الطلبات بدونه —
   إن تُرك فارغًا يبقى الاستقبال شغّالًا بينما يفشل الإرسال بصمت (401).
2. `docker compose --profile dev up -d waha` ثم افتح `http://localhost:3210`
   وامسح رمز QR بجوال الفندق.
3. الجلسة محفوظة في volume باسم `waha-sessions` — لا يلزم مسح QR بعد كل تشغيل.
4. تحقق: `curl -H "X-Api-Key: $WAHA_API_KEY" http://localhost:3210/api/sessions`
   يجب أن تكون `"status":"WORKING"`.

## ربط Google (التقييمات)
- تجريبي: زر «ربط حساب تجريبي» في شاشة السمعة الرقمية (accountRef يبدأ بـ `mock:`).
- إنتاج: `POST /api/reputation/link` مع `accountRef` (مسار الموقع في GBP) و`oauthRefreshToken`
  — التوكن يذهب للخزنة، ويتطلب `GOOGLE_OAUTH_CLIENT_ID/SECRET` في البيئة.

## ربط الشبكات الاجتماعية (النشر)
- تجريبي: `SOCIAL_PUBLISHER=mock` (الافتراضي) — روابط نشر وهمية مستقرة.
- إنتاج: خزّن `meta.pageToken` و `meta.pageId` في الخزنة واضبط `SOCIAL_PUBLISHER=meta`.
- تيك توك يبقى draft-mode (§8) حتى يتوفر مسار API.

## إدارة التخزين والمستخدمين
- الحصة لكل فندق في `Tenant.quotaGb`؛ التنبيه عند 80% تلقائي، والرفض عند 100% (507).
- سلة المحذوفات 30 يومًا ثم حذف نهائي (مكنسة كل ساعة).
- إضافة موظف: صف `StaffMember` بالدور المناسب (§3) — الأدوار في
  `apps/api/src/modules/auth/permissions.ts` وهي الجدول الوحيد للصلاحيات.

## تجهيز عرض تجريبي نظيف
قبل أي عرض على عميل، أرجع بيانات الفندقين التجريبيين لحالة نظيفة:
```bash
docker compose exec api npm run db:reset-demo && docker compose exec api npm run db:seed
```
يمسح المحادثات والتذاكر وأوامر العمل والضجيج التشغيلي فقط — الموظفون
والمعرفة وهوية العلامة وسياسات الوكلاء تبقى، وسجل التدقيق لا يُمس أبدًا
(سلسلة مقاومة للعبث تُقتطع عند الحاجة ليست كذلك). يرفض العمل على قاعدة
غير محلية بدون `--force`.

## بيانات العرض التجريبي
لعرض المنصة وهي "مستخدَمة" منذ ثلاثة أسابيع بدل شاشات فارغة:
```bash
docker compose exec api npm run db:reset-demo && docker compose exec api npm run db:seed && docker compose exec api npm run db:demo-history
```
يزرع محادثات واتساب حقيقية، تذاكر بمهل (بعضها متجاوز)، أوامر عمل بينها
عطل حرج بانتظار تأكيد المدير، تقييمات Google بعضها مُجاب، قنوات تواصل
بأرقامها، محتوى عبر كل المراحل، حالات شكاوى بتحليل سبب جذري متكرر،
وأحداث تملأ مركز العمليات. المؤشرات تُحسب من هذه البيانات فعليًا،
وسجل التدقيق يبقى صالحًا (`/api/audit/verify`).

**ملاحظة:** القنوات تُزرع مفعّلة لكن **غير موصولة** — الربط يعني رمزًا
حقيقيًا في الخزنة، وتزييفه يجعل الوكيل يدّعي قدرة لا يملكها. اترك الربط
خطوة حيّة في العرض.

## عمليات يومية
- تعطّل وكيل؟ مركز الوكلاء → إيقاف — الرسائل تتحول للموظفين فورًا.
- استلام محادثة من AI: صندوق الرسائل → «استلام المحادثة»؛ الإرجاع للـ AI قرار مدير.
- التحقق من سلامة سجل التدقيق: `GET /api/audit/verify` — يجب `valid: true` دائمًا.
